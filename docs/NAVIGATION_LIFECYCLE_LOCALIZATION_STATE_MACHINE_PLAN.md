...
> 状态：实施中（核心门禁已完成；Lifecycle configure-only、直接入口统一与RTK搜索种子正在补齐；现场联合验收待按安全场地执行）
> 保存日期：2026-09-16  
...
- 进行中：Nav2地图/安全组与执行组分离；任务启动已采用 `prepare → 定位 → activate_execution → Action验收`，正在补齐“执行组显式configure且inactive”与失败回滚验证。
- 进行中：人工初始位姿、主动重定位、定位丢失恢复与分地图切换已进入 NDT 优先事务；正在将直接 `nav.start/restart/recover` 与RTK fixed可信搜索种子完全并入同一编排。
- 已完成：路径规划定位时间线补入 prepare、FAST-LIO 收敛、最终放行和执行组激活，并保留后端实际时间证据。
- 已验证：`mapping` 目标构建成功；此前Edge的导航栈适配、命令处理、分地图和启动定位定向测试，以及前端 183 项测试和生产构建通过。本轮变更须在提交前重新完成相应定向测试。
