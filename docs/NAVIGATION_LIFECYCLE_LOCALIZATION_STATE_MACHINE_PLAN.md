...
> 状态：已完成（代码与定向验收完成；现场联合验收待按安全场地执行）  
> 保存日期：2026-09-16  
...
- 已完成：Nav2地图/安全组与执行组分离；任务启动顺序为 `prepare → 定位 → activate_execution → Action验收`。
- 已完成：人工初始位姿、主动重定位、定位丢失恢复与分地图切换均进入 NDT 优先、执行组暂停/恢复事务；分地图不再走 full-stop 重启。
- 已完成：路径规划定位时间线补入 prepare、FAST-LIO 收敛、最终放行和执行组激活，并保留后端实际时间证据。
- 已验证：`mapping` 目标构建成功；Edge 的导航栈适配、命令处理、分地图和启动定位定向测试，以及前端 183 项测试和生产构建通过。
