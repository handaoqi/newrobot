...
- [x] 任务启动、前端地图激活闭环、主动重定位和自动恢复均先进入 NDT 搜索/提交与 FAST-LIO 接管，不再保留直接 RTK 初始位姿旁路。
- [x] 前端地图激活和路线保存将首航点 `localization_mode` 作为 RTK/UKF/NDT 二次校正策略透传；未显式传入时从首航点自动提取。
- [x] 自动恢复顺序固定为当前航点、最近可信位姿、相邻/端点候选、全局搜索；每次尝试前重新读取当前航点校正策略。
- [x] 局部和全局 NDT 恢复成功后统一执行二次校正并显式解除任务定位暂停；激光特征搜索成功只触发下一轮 NDT 提交，不再直接恢复任务。
- [x] 二次校正使用消息/任务唯一事务 ID，等待明确终态；拒绝、失败和超时不再伪报定位成功，超时事务会取消并携带诊断结果。
- [x] 初始化结果向任务进度回传 `initial_ndt_commit`、`fast_lio_imu_handoff`、`secondary_correction` 和 `continuous_source=lio_imu`。
- [x] 路径规划页阶段顺序统一为“候选搜索 → 最优 NDT 提交 → FAST-LIO + IMU 接管 → RTK/UKF/NDT 二次校正 → 导航启动”，旧 RTK 优先测试契约已更新。
- [x] 航点播报默认启用全局阻塞策略，具体是否阻塞仍由航点 `blocking/non_blocking` 配置决定。
