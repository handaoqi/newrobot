...
当前工程已经具备部分基础能力，但“锁原点操作流程”尚未形成完整的前端—后端—Edge—SLAM 状态机。实施时必须区分已具备能力和待开发能力：

| 能力 | 当前状态 | 说明 |
|---|---|---|
| `indoor` / `outdoor` 建图类型 | 已有协议和场景约束基础 | `local_only` 地图只能用于室内 NDT；室外/过渡区要求 RTK 固定地图 |
...
| 室外锁原点 60 秒门控 | 计划实现 | 本文定义完整状态机、接口和验收标准 |
