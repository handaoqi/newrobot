...
| 室外锁原点 60 秒门控 | 已实现，待实机验收 | 三话题质量证据、连续计时重置、30 分钟 TTL、断电恢复和航向复核已接通 |
...
```text
准备传感器 → 静止采样 60 秒 → 锁定原点 → 启动 FAST-LIO-SAM（不建图）→ 人工原地转动复核 → 开始建图
```
...
| `启动传感器并锁原点` | 室外初始阶段 | 启动雷达、IMU、双天线 RTK，进入 60 秒原点质量采样；不启动 FAST-LIO-SAM |
| `锁定 ENU 原点`（新增） | 室外采样质量合格后 | 固化原点、生成 `gnss_origin.yaml` 和 `origin_lock_session_id` |
...
- 室外未满足 60 秒质量条件时，“锁定 ENU 原点”禁用。
- 室外未完成原点锁定时，不能启动 FAST-LIO-SAM。
- 室外没有 SLAM 健康状态和新鲜双天线航向时，“确认航向稳定，开始建图”禁用。
...
- 60 秒、2cm、双天线基线和航向精度作为室外建图默认配置项，默认值分别为 `60s`、`0.02m`、`0.20m`、`5°`。
...
建议新增独立模块 `origin_lock_monitor.py`，不要把 60 秒采样逻辑散落在 `mapping_adapter.py` 的启动和保存函数中。
...
3. `COLLECTING_STABLE_WINDOW`：开始 60 秒连续计时，实时更新样本、散布和质量状态。
...
| 场景 | 必须动作 | 禁止动作 |
|---|---|---|
| RTK 未 FIX | 不开始 60 秒计时，显示等待原因 | 不使用单点或历史坐标锁原点 |
...
- 前端：按钮门控、倒计时、实时状态、失败原因和地图详情。
- 地图包：manifest 字段、原点文件、raw/optimized 轨迹、回环状态和完整性计数。
...
1. 原点锁定会话状态为 `locked`，且 60 秒质量证据完整。
...
- Edge：`origin_lock.py` 实现 `/fix`、`/rtk_pvh`、`/rtk/ntrip_status` 三源质量采样，60 秒连续窗口、2 cm 最大水平散布、失败清零、原点原子落盘和 TTL 恢复。
...
- 前端：增加室内/室外模式卡、室外原点实时质量面板、状态驱动按钮门控、动态完整状态机和响应式操作布局；保留停止保存、取消和刷新操作。
- 恢复：`mapping_workflow.json` 持久化工作流。Edge 重启后只恢复 TTL 内的锁定原点，航向稳定状态必须重新实时采样，不继承旧确认。

软件验证结果：Edge 全量测试 128 项通过；前端 11 项测试和生产构建通过；`robot_slam` ROS2 包 Release 编译通过；后端 `manage.py check`、迁移一致性检查和新增的 9 项协议/REST 测试通过。

尚未由软件测试替代的工作只有现场验收：真实双天线 RTK 连续 60 秒锁定、原地转动航向复核、室内一次完整建图、室外一次完整建图，以及保存后重新加载地图。现场证据必须填写第 7 节模板。
