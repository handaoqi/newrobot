...
| 14 | 基础完成（代码/单测） | 新增 `RecoveryArbiter` 单一恢复所有权和 generation，先接入 Edge 障碍后退恢复；Nav2 BT、定位恢复和侧移恢复仍需继续接入同一仲裁器。 |
| 15 | 已完成（代码/单测） | 到点稳定等待在新遥测存在 `sample_age_seconds` 时拒绝超过 1.5 秒的旧定位样本；旧版无年龄字段时保持兼容。 |
| 16 | 基础完成（代码/单测） | 引入不可变 `LegProfile` 和 `leg_generation`，发车事件携带实际路段配置；安全层保持 `collision_stop_enabled=true`，完整 capability 读回/回滚仍待设备接口。 |
