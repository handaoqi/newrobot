...
- 状态：分阶段实施中（P0–P2 主链路与本轮可代码闭环项已收口；现场四种算法组合验收、平台 capability 动态选项、完整 docking 接触闭环、Nav2 BT 恢复所有权硬互斥仍待设备侧验证）。
...
### 1.3 2026-09-10 终点转圈修复执行记录

- 已完成：终态控制边界收口。任务进入完成或失败前，统一使旧 Nav2 回调失效、清理 departure heading，并发布显式零速度。
- 已完成：`RosAdapter.stop_motion()` 改为持续约 0.5 秒重复发布零速度，完成/取消/失败路径可幂等调用。
- 已完成：终点接近段 MPPI 参数收紧为 `vx_min=0`、`wz_max=0.35`，禁止终点附近倒车和高角速度绕行；恢复动作仍由独立仲裁器负责。
- 已完成：终点 MPPI profile 强制下发 `GoalCritic.enabled=true`、`vx_min=0`，并在 Nav2 参数服务读回不一致或下发失败时进入安全保持，禁止带着巡航 profile 继续前进。
- 已完成：循环换轮首段不再等待 departure-heading 线程；先执行反向起点共点跳过，再立即下发首个单航点，避免定位航向未刷新时整轮无 action。
- 已完成：保安值守页面的休息期导航维护增加 5 秒换轮闸门；维护 Promise 卡住但设备实际已就绪时，废弃旧页面代次并立即下发下一轮；设备未就绪时回到可见的短间隔重试，不再无限显示“正在启动下一轮”。
- 已验证：Edge 导航与任务状态机回归测试 128 项通过。
- 保持不变：FAST-LIO 原始位姿不作为 Nav2 控制位姿；现有 `/localization/decision` 异常闸门和 `RecoveryArbiter` 继续负责漂移/重定位仲裁。
- 待设备验证：1 号点现场不再绕行、旧控制命令完全清零，以及 MPPI/RPP 参数优化效果。参数调整必须在现场闭环验证后单独提交。
...
| 9 | 已完成（代码/单测） | 到点确认连续新鲜位姿帧门控；`LegProfile` 含 `speed_profile`/三层避障/goal checker；事件幂等键 `(task, round, waypoint, leg_generation, event_type)`。 |
...
| 12 | 已完成（代码/单测） | 补齐重定位候选传递与旧版 `quick_then_global` 协议兼容；守卫任务把路线航点传入定位流程；MQTT 命令异常隔离；未激活地图时边界观测返回完整安全状态。 |
| 13 | 已完成（代码/单测） | 引入 `arrival_policy`（`pass_through`、`stop_and_confirm`、`precision`、`dock`）的路线归一化、协议校验和页面配置；明确 `pass_through` 不触发静止校正、停留、动作或到点语音。 |
| 14 | 已完成（代码/单测） | `RecoveryArbiter` 含 generation 与有限预算；Edge 障碍后退/侧移与定位恢复均接入；预算耗尽进入 `safe_hold`。Nav2 BT 恢复仍为并行观察项，尚未做进程间硬租约。 |
| 15 | 已完成（代码/单测） | 到点稳定等待在新遥测存在 `sample_age_seconds` 时拒绝超过 1.5 秒的旧定位样本；旧版无年龄字段时保持兼容。 |
| 16 | 已完成（代码/单测） | 不可变 `LegProfile` + `leg_generation`；`set_safety_profile` 独立下发绕行/减速/硬停；硬停强制开启。完整 capability 协商服务仍待设备接口。 |
| 17 | 已完成（代码/单测） | `stop_and_confirm`/`precision`/`dock` 门控；室内校正后点击点复核；`arrival_confirmed` 进度事件；`speech_mode`；航点 `snapshot` 动作注册表。 |
| 18 | 已完成（代码/单测） | 分级预转向（约 10°/60°）与连续 yaw 样本完成判定；单点 BT 对齐多点 `ReinitializeGlobalLocalization`。 |

### 1.2 与当前代码的剩余差异（2026-09-07）

| 项目 | 状态 |
| --- | --- |
| 四种算法组合现场读回验收 | **已完成（2026-09-07）**：设备注册 `ThetaStar/NavFn`+`FollowPath/RPP`；脚本四种组合写参读回与 selector 发布均通过。巡检实跑读回仍建议各跑一条短线确认。 |
| `ApplyNavigationProfile` 单一原子服务 + 能力协商 + 失败回滚 | **代码已完成**：`ros_adapter.apply_navigation_profile` 含读回与回滚；`list_navigation_capabilities` 可报静态+运行时插件。需重启 Edge 后在任务中生效。 |
| Nav2 BT 恢复与 Edge 的进程级硬互斥租约 | 部分：Edge 侧仲裁完整；BT 仍可独立旋转/后退 |
| 侧移点 `ComputePathToPose`/足迹/地图边界完整校验 | 部分：侧向净空启发式；无全局路径可达性预检 |
| 校正 generation 与定位节点事务字段双向绑定 | 部分：Edge 维护 `correction_generation`；定位节点尚未回传匹配字段 |
| 平台按机器人 capability 动态显示算法选项 | 未做（静态四选项） |
| `dock` 接触/充电闭环现场验收 | 待现场 |
| dwell ±0.2s 现场误差测量 | 待现场 |
...
建议按“可观测性与名实修正 → 到点/校正事务 → 段配置与转向 → 自愈仲裁 → 灰度验证”实施，避免一次性改动整个导航链路。本轮已继续完成重定位输入链路和异常隔离；下一优先级仍是 P2 的到点/校正事务，不得把本轮协议兼容改动视为到点闭环已经完成。
...
- 已完成：引入航点 `arrival_policy`，并对历史未带字段的航点采用安全的 `stop_and_confirm` 默认值。
- 已完成：停车类策略在校正后重新读取位姿，要求连续新鲜样本；`precision` 采用更严格的 XY/yaw 门限，失败不推进下一段。
- 落地到点状态机和幂等键。
- 室内增加校正后点击点复核。
- 已完成：`dock` 策略字段与 `context.docking.enabled` 做发车前强绑定；接触/充电确认失败策略仍需现场闭环验证。
- 明确普通、精确、室外和停靠失败策略。
