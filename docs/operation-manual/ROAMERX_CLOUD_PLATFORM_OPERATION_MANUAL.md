...
- 已对保安值守、监测中心、远程控制和统计分析页执行刷新后等待 20 秒的复采集，截图均已返回实时任务、设备、告警或统计数据。视频区域显示“无信号”或加载状态时，仍须到“机器人管理”核实设备与流媒体链路，不能据此判断机器人安全。
...
1. 选择左侧“保安值守”。
2. 先确认页面框架、任务状态、地图和报警区域已经出现；视频区域会单独显示“正在加载视频流”。
3. 视频成功后再确认画面、设备名称和位置；若持续加载或变为无信号，转到“机器人管理”确认设备在线，并核对 NX Edge Agent 与云端连接，不要反复点击启动按钮。
...
监测中心用于汇总实时视频、告警、设备电量与人工处置能力。本图已在刷新页面后等待 20 秒采集；若视频区显示“无信号”，表示当前视频源不可用，不代表设备离线。

### 日常查看

1. 选择“监测中心”；页面摘要和操作卡片会先出现，视频区域随后显示“正在加载视频流”。
...
1. 打开“巡检任务 → 路径规划”。主卡片顶部显示步骤 1—4；点击“收起/展开”可压缩地图上方的配置区。
2. 在步骤 1 选择地图，在步骤 2 填写路线信息，在步骤 3 点击地图添加途经点并设置方向、定位方式、避障和巡检播报。
3. 删除途经点时点击该点右侧红色“删除”按钮；“清空”会删除全部当前未保存途经点。
4. 地图区域位于主要工作区，步骤 5“导航测试”位于主卡片右下方。导航初始化、启动、停止、执行和急停仍需现场授权。
