...
当前紧急停止按钮已接入后端控制接口。Demo 中该按钮下发 `shake_hand` 动作，用于验证“前端按钮 -> 后端 -> 板端 -> 机器狗 SDK”的完整链路；生产环境应将真正急停映射为 `passive` 或 `move_stop`。
...
当前喊话、语音预览为前端演示状态，仅展示 Toast，不向后端或板端发送控制指令。
...
| FLV 不可播放 | 尝试 HLS |
| 没有实时流 | 显示静态图片 |
| 控制类按钮 | 紧急停止 demo 会落库并下发到板端，喊话和语音预览仍为 Toast 演示 |

## 10. 可扩展功能建议

后续若从演示升级到真实业务，可补充：

- 远程控制接口：喊话、紧急停止、返航、接管。
