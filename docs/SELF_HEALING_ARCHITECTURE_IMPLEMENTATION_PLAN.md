...
- [ ] systemd 服务重启和 NX 重启后仍从同一 `current-edge` release 启动；
- [ ] release 切换失败可自动回滚，开发工作区未提交文件不进入运行进程；
- [ ] 恢复后先执行原地转向时，迟到恢复命令不增加取消次数或停车命令次数；
- [ ] 实际速度样本为空、缺字段、非法或与定位同时过期时全部保持安全阻塞；
- [ ] 新鲜零速连续观察五秒可恢复，真实非零速度仍返回 `ROBOT_NOT_STOPPED`；
- [ ] `task.recover.v1` 失败不终结任务，`task.force_exit` 的入口和终态语义不变；
- [ ] Edge 定向测试、平台循环与消息处理测试通过，部署后服务和版本路径核验通过。
