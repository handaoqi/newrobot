...
| 视口 | 布局策略 | 固定验收 |
| --- | --- | --- |
| 移动设备且 portrait | 右上角圆形菜单按钮 + 右侧抽屉导航，内容按可读顺序流式重排 | `768×1024` WebKit、`800×1280` Chromium、`834×1194` WebKit |
| 桌面设备（任意宽度）或移动设备 landscape | 始终保留左侧竖向菜单，不切换成横向菜单或抽屉 | `800×1000` 窄桌面、`1200×800` 移动横屏、`2000×1200` Chromium |
| 桌面/手机 | 保持独立的现有桌面和手机断点 | 不因平板规则改变 |

工作台先通过 User-Agent Client Hints、Android/iOS/iPadOS 标识判断移动设备，再结合屏幕方向选择导航模式，不以视口宽度或 `pointer: coarse` 单独决定导航。这样桌面浏览器缩窄时仍保持左侧菜单，触控平板横屏时也不会出现中间态横向菜单。
...
测试使用 `tests/tablet/mock-api.js` 固定 API 数据和静默 EventSource，不启动 Django、ZLMediaKit、Edge 或真实机器人。布局报告检查页面和可见元素边界、全局横向滚动、触控尺寸；移动竖屏路由额外检查圆形菜单按钮、抽屉打开/关闭、Esc、aria 状态和焦点恢复。测试还覆盖窄桌面与移动横屏保持左侧竖向菜单，并在 2000×1200 项目检查所有工作台路由的壳层不越界。
