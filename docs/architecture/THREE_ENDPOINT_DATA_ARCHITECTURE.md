...
浏览器前端不直接访问上述数据库。监测中心、保安值守和巡检任务页面通过 `src/services/api.js` 读取 API 返回的摘要、地图、路线、任务和视频地址；实时告警使用 SSE，视频使用 API 返回的 FLV/HLS 地址。页面先渲染结构，再异步加载数据和视频，避免媒体服务变慢时阻塞整个工作台。
...
- 12 个监测中心播报模板。
- 5 个告警技能绑定：自行车、发现障碍、避障、劝阻、低电量停车告警。
...
| 设备管控 | 低电量停车告警 | 当前电量低于百分之二十，导航任务已停止，请及时人工处理或手动回充。 |

迁移 `0054_seed_eleven_speech_templates` 固化首批内容，后续迁移补充低电量停车告警。运行 `initialize_platform` 只补缺失记录，不覆盖页面已经修改的正文或技能绑定；迁移 `0070` 清理早期已废弃的“低电量自动回充”模板。
